...
#comds$city = ciesties$city where comds$placeID = cities$placeID
for(i in 1:nrow(latlong)) {
    row <- latlong[i,]
    print("Searching city, row:")
    print(row$city)
    cities <- revgeo(row$longitude, row$latitude,  output = 'hash', item ='city')
    print("Setting City Found:")
    print(cities)
    comds$city[which(comds$placeID == row$placeID)] <- as.character(cities)
    print("Latlong new city:")
    #latlong$city[i] <- cities$city
    # do stuff with row
}
