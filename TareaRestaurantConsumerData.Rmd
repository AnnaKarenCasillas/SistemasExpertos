...
tops <- which(mis_recomendaciones[]>0.5 & mis_recomendaciones[] < 1.0, arr.ind = TRUE) 
#nombres[tops]
...
```
```{r}
DF2 = as.data.frame(top_recomendaciones)
Resultado <- subset(comds, placeID %in% DF2$V2) 

recomend <- unique(Resultado$name)
paste(c("Te recomendamos los 10 siguientes lugares cerca de tí:", recomend))
```
