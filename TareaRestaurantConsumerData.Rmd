...
rating <- read.delim("/Users/akcasill/Documents/Maestria/RCdata/rating_final.csv", sep = ",", stringsAsFactors = TRUE, header=TRUE)
str(rating)
head(rating)
```
...
```{r}
#Cargamos Geoplaces
geoplaces <- read.delim("/Users/akcasill/Documents/Maestria/RCdata/geoplaces2.csv", sep = ",", stringsAsFactors = FALSE, header=TRUE)
str(geoplaces)
head(geoplaces)
```
...
chfcn <- read.delim("/Users/akcasill/Documents/Maestria/RCdata/chefmozcuisine.csv", sep = ",", stringsAsFactors = TRUE, header=TRUE)
str(chfcn)
head(chfcn)
```
...
### Buscamos ciudades para los registros que no tienen o están marcados con "?"
...
cities <- revgeo(latlong$longitude, latlong$latitude,  output = 'frame')
cities
...
### Corroboramos que ya no hay regitros sin ciudad
...
```
### Vamos a agrupar las ciudades repetidas 
...
###Volvemos a imprimir las ciudades
